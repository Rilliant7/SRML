...
from models.Conv_4 import BackBone
...
class RENet(nn.Module):
    ...
    def forward(self, input):
        ...
        elif self.mode == 'resnet12':
            return self.resnet12(input, False)
        elif self.mode == 'scr_clf':
            spt, qry = input
            return self.scr_classifier(spt, qry)
        else:
    ...
    def encode(self, x, do_gap=True):
        x = self.encoder(x)#经过一个resnet12
        if self.args.self_method:
            identity = x
            x = self.scr_module(x)
            if self.args.self_method == 'scr':
                x = x + identity
            x = F.relu(x, inplace=True)#F_q
    ...
    def resnet12(self, x, do_gap=True):
        x = self.encoder(x)#经过一个resnet12
        if do_gap:
            return F.adaptive_avg_pool2d(x, 1)
        else:
    ...
    def scr_classifier(self, spt, qry):
        ...
        way = spt.size(0)
        num_qry = qry.size(0)
        # Repeat spt and qry_sim according to the number of shots
        spt = spt.unsqueeze(0).repeat(num_qry, 1, 1, 1, 1)
        qry_sim = qry.unsqueeze(1).repeat(1, way, 1, 1, 1)
        if self.args.shot > 1:
            spt = spt.view(num_qry, self.args.shot, self.args.way, *spt.shape[2:])
            qry_sim = qry_sim.view(num_qry, self.args.shot, self.args.way, *qry_sim.shape[2:])
            spt = spt.mean(dim=1)
            qry_sim = qry_sim.mean(dim=1)
        # Calculate mean embeddings for spt and qry_sim
        spt_mean = spt.mean(dim=[-1, -2])
        qry_mean = qry_sim.mean(dim=[-1, -2])
        # Calculate pooled embeddings for qry
        qry_pooled = qry.mean(dim=[-1, -2])
        # Calculate cosine similarity between mean embeddings
        similarity_matrix = F.cosine_similarity(spt_mean, qry_mean, dim=-1)
